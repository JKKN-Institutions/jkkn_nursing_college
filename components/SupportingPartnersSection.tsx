...
import { supportingPartners } from "@/data/partnersData";

export default function SupportingPartnersSection() {
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 5;
  const totalPages = Math.ceil(supportingPartners.length / itemsPerPage);
        ...
        <div className="relative mt-12">
          ...
          >
            {supportingPartners.map((partner) => (
              <div
                key={partner.id}
                className="flex-shrink-0 w-[220px] h-[160px] bg-white border border-gray-200 rounded-lg flex items-center justify-center p-6 hover:shadow-lg transition-shadow"
              >
                <Image
                  src={partner.logo}
                  alt={partner.name}
